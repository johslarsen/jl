...
static size_t sum_after_xor(std::string_view key, std::string_view plaintext) {
  size_t sum = 0;
...
static void BM_StringViewXorSum(benchmark::State& state, std::string_view str, std::string_view key) {
  size_t sum = -1;
  for (auto _ : state) {
