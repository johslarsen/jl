...
TEST_SUITE("time") {
  ...
  TEST_CASE("clock conversion") {
    ...
    SUBCASE("conversion from UTC to TAI is consistent") {
      CHECK("2017-01-01 00:00:35" == std::format("{}", clock_cast<tai_clock>(last_leap - 1s)));
      CHECK("2017-01-01 00:00:36" == std::format("{}", clock_cast<tai_clock>(last_leap)));
      CHECK("2017-01-01 00:00:37" == std::format("{}", clock_cast<tai_clock>(last_leap + 1s)));
    }

    SUBCASE("conversion from TAI to UTC is consistent") {
      auto two_to_leap = clock_cast<tai_clock>(last_leap - 2s);
      CHECK("2016-12-31 23:59:58" == std::format("{}", clock_cast<utc_clock>(two_to_leap)));
      CHECK("2016-12-31 23:59:59" == std::format("{}", clock_cast<utc_clock>(two_to_leap + 1s)));
      CHECK("2016-12-31 23:59:60" == std::format("{}", clock_cast<utc_clock>(two_to_leap + 2s)));
      CHECK("2017-01-01 00:00:00" == std::format("{}", clock_cast<utc_clock>(two_to_leap + 3s)));
    }
